...
class categorization(BaseModel):
    ...
    job_function_prompt: str = """Role: You are a technical recruiter.
...
"""
    domain_preference_prompt: str = """Objective: As a technical recruiter, you're tasked with evaluating a candidate's expertise in their technical domain based on their resume (‘resume_text’ and/or ‘linkedin_text’) and present/past employers' descriptions (‘company_description’). Your goal is to identify the primary technical domain and up to three subdomains they have experience in, guided by their job responsibilities and the nature of their past employment.
...
{ "domain_analysis": [ {"job_title": "", "job": "[Job Title] at [Company Name]", "identified_domain": "[Domain]", "subdomain": [ "[Subdomain 1]", "[Subdomain 2]", "[Subdomain 3]" ] } // Repeat for each additional position ]}"""
    ...
    industry_experience_prompt: str = """Role: As a technical recruiter, evaluate a candidate's industry experience based on their resume (‘resume_text’ and/or ‘linkedin_text’) and past employers' descriptions (‘company_description’). Use the ‘IndustryList’ below to accurately align the candidate's background with specific industry criteria. For every role in the resume, including multiple positions at the same company, assess each job title independently to comprehensively capture the candidate's industry experience across all roles.
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if categorizer.job_function_extraction:
                futures.append(executor.submit(response_obj.job_function, prompt = categorizer.job_function_prompt))
            if categorizer.domain_categorization:
                futures.append(executor.submit(response_obj.domain_categorization, prompt = categorizer.domain_preference_prompt, company_description=companies_description))
            if categorizer.industry_experience_extraction:
                futures.append(executor.submit(response_obj.industry_experience, prompt = categorizer.industry_experience_prompt, company_description=companies_description))
